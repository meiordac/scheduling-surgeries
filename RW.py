...
class ReadExcel:
  ...
  def Read():
    ...
    for row_index in range(1,sheet.nrows):
      rows.append({"Name":sheet.cell_value(row_index,0),"Date":sheet.cell_value(row_index,1),"Age":sheet.cell_value(row_index,2), "Rut":sheet.cell_value(row_index,3), "PatientType":sheet.cell_value(row_index,12),"Patient":sheet.cell_value(row_index,13),"GES":sheet.cell_value(row_index,14),"SurgeryLength":sheet.cell_value(row_index,15),"anesthesiaType":sheet.cell_value(row_index,16) })
    return rows
  ...
  def generateExcel(pr, rows):
    ...
    i = len(pr.patients) + 3*(len(pr.days))*(len(pr.blocks)) + (len(pr.days))+(len(pr.days))*(len(pr.operatingRooms)) + 3  #primera linea son cantidad de variables, iteraciones y luego valor de FO
