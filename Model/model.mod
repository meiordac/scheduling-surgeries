...
param patientTypeByPatient{p in patients};
...
var assignedPatient{p in patients, pt in patientTypes, b in blocks, d in days} binary;
...
maximize objectiveFunction:
  sum{pt in patientTypes, b in blocks, d in days, p in patients, op in operatingRooms} (assignedBlock[pt,op,b,d]*alpha[pt] + assignedPatient[p,pt,b,d]*waitingPeriod[p]+assignedPatient[p,pt,b,d]*beta*ges[p]); 
...
subject to rest_no_duplicate_patients {p in patients}: 
  sum{d in days,pt in patientTypes, b in blocks} assignedPatient[p,pt,b,d] <= 1;
...
subject to rest_max_num_patients_per_block {b in blocks, d in days}:
  sum{p in patients, pt in patientTypes} assignedPatient[p,pt,b,d]*surgeryLength[p] <=  blockDuration[b];
...
#subject to corresponding_block_patient{d in days, pt in patientTypes, p in patients, b in blocks, op in operatingRooms}:
  #assignedPatient[p,pt,b,d] <= assignedBlock[pt,op,b,d];
  
subject to rest_anesthesists{d in days, b in blocks}:
  sum{p in patients, pt in patientTypes} assignedPatient[p,pt,b,d]*anesthesiaType[p] <= anesthetists[b,d];
